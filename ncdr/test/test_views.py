...
from ncdr.models import Column, Database, DataElement, Grouping, Schema, Table, User
from ncdr.search import searchable_models
...
class AbstractViewTestCase(TestCase):
    ...
    def create_csv_column(
        ...
        table, _ = Table.objects.get_or_create(name=table_name, schema=schema)
        if not column_args:
            column_args = {}
        ...
        column = Column.objects.create(**default_args)
        return column
...
class PublishAllTestCase(AbstractViewTestCase):
    def test_publish_all(self):
        ...
        self.assertEqual(result.url, "/")
        self.assertTrue(Column.objects.get(id=column.id).published)
...
class NCDREditViewTestCase(AbstractForm, AbstractViewTestCase):
    def setUp(self):
        super().setUp()
        column = self.create_csv_column()
        data_element = DataElement.objects.create(name="data_element", column=column)
        Grouping.objects.create(name="grouping", dataelement=data_element)
...
class NCDRSearchRedirect(AbstractViewTestCase):
    ...
    def test_get_with_database(self):
        ...
        """
        database_name = Database.objects.get().name
        column_name = Column.objects.get().name
...
class DataElementDetailTestCase(AbstractViewTestCase):
    def setUp(self):
        ...
        self.data_element = DataElement.objects.create(
...
class GroupingListViewTestCase(AbstractViewTestCase):
    ...
    def test_get_populated(self):
        ...
        self.column.data_element = self.data_element
        self.column.save()
        self.grouping = Grouping.objects.create(name="grouping")
